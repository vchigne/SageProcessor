// API para desplegar un servidor DuckDB
// Este endpoint envía una solicitud al API de DuckDB Swarm para desplegar un servidor
...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      if (server.is_local) {
        return res.status(400).json({ 
          error: 'No se puede desplegar el servidor local' 
        });
      }
      ...
      // Llamar al API de DuckDB Swarm para desplegar
      try {
        console.log("Enviando datos al API:", JSON.stringify({
          ...
          redeploy: true // Indicamos que es un redespliegue/despliegue
        }));
        ...
        if (!deployResponse.ok) {
          ...
          return res.status(deployResponse.status).json({
            error: `Error al desplegar: ${errorData.message || 'Error desconocido'}`
          });
        }
        ...
        if (!deployResult.success) {
          ...
          return res.status(500).json({ 
            error: `Error al desplegar: ${deployResult.message || 'Error desconocido'}`
          });
        }
    ...
    return res.status(500).json({ error: 'Error al desplegar servidor: ' + error.message });
  }
}