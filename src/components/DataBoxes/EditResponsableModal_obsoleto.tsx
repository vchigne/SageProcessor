...
export const EditResponsableModal: React.FC<EditResponsableModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
  responsableData
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (responsableData?.id) {
        console.log("Editar responsable:", responsableData);
        setFormData({
          ...
          configuracion_frecuencia: {
            ...
            dias_semana: responsableData.configuracion_frecuencia?.dias_semana || [],
            dias_mes: responsableData.configuracion_frecuencia?.dias_mes || []
          },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Incluir el ID para edición
      const dataToSubmit = {
        id: formData.id, // ID siempre presente para edición
      ...
      alert(error.message || `Error al editar el responsable. Por favor verifique los datos e intente nuevamente.`);
    } finally {
      setEnviando(false);
    }
  };
