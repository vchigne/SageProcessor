...
# Script mejorado para iniciar servicios VNC para DuckDB UI
# Este script configura un entorno gráfico XFCE4 completo con soporte VNC
...
echo "[+] Iniciando servicios VNC con configuración mejorada"
...
echo "[*] Configurando entorno gráfico XFCE4..."
...
# --- Iniciar VNC con opciones mejoradas
...
if ! ss -ltn | grep -q ":${VNC_PORT}"; then
    echo "[!] Advertencia: VNC no está escuchando en el puerto ${VNC_PORT}, intentando método alternativo..."
...
if ss -ltn | grep -q ":${VNC_PORT}"; then
    ...
    echo "[✗] VNC NO está escuchando en el puerto ${VNC_PORT}, revisa logs en ${LOG_FILE}"
fi
...
if [ ${VNC_RUNNING} -eq 1 ] && [ ${WEBSOCKIFY_RUNNING} -eq 1 ]; then
    ...
    echo "[!] Servicios VNC iniciados parcialmente - Revisa los logs en ${LOG_FILE}"
