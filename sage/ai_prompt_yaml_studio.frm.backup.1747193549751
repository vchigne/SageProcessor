...
2. Para archivos sin cabeceras, SIEMPRE usa nombres column(NN) para los campos, empezando desde 00, nunca "Unnamed: X". Es decir, por ejemplo, el primer campo sera column00, el segundo sera column01, el tercero sera column02 y asi sucesivamente.
...
6. Para el Name del sage_yaml trata de ser lo mas descriptivo posible, no uses solo un numero sino lo que hayas podido inferir del contendo del catalogo, lo mismo que para Description. Alli puedes poner tambien la descripcion de las reglas especificas de validacion que vas a generar, tal ez algun FAQ o algo sobre ese catalogo, etc. Que no sea muy grande pero si informativo, porque es la base del lineage y de la gobernanza de datos
...
3. **packages**: Con la definición de al menos un paquete que agrupe los catálogos. Cada paquete DEBE contener:
   ...
   - file_format: Debe incluir 'type: ZIP' para paquetes con múltiples catálogos o si es un solo catalogo puede ser ZIP, CSV o EXCEL
   - catalogs: Lista de los IDs de catálogos que incluye el package
